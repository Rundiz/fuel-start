<h1><?php echo __('accountlv_role'); ?></h1>

<div class="row cmds">
    <div class="col-sm-6">
        <ul class="actions-inline">
            <li><?php printf(__('admin_total', array('total' => (isset($list_levels['total']) ? $list_levels['total'] : '0')))); ?></li>
            <?php if (\Model_AccountLevelPermission::checkAdminPermission('accountlv_perm', 'accountlv_add_perm')) { ?><li><?php echo \Html::anchor('admin/account-level/add', __('admin_add'), array('class' => 'btn btn-default')); ?></li><?php } ?> 
        </ul>
    </div>
</div>
        ...
        <table class="table table-striped table-hover list-logins-table table-sortable">
            <thead>
                <tr>
                    ...
                    <th><?php echo __('accountlv_level_priority'); ?> <span class="glyphicon glyphicon-question-sign bootstrap-tooltip" data-toggle="tooltip" data-original-title="<?php echo __('accountlv_higher_priority_will_come_first'); ?>"></span></th>
                    <th><?php echo __('accountlv_role'); ?></th>
                    <th><?php echo __('accountlv_description'); ?></th>
                    <th></th>
                </tr>
            </thead>
            <tfoot>
            ...
            <tbody>
                ...
                <tr class="state-default<?php if (in_array($row->level_group_id, $disallowed_edit_delete)) { ?> ui-state-disabled<?php } ?>" id="listItem_<?php echo $row->level_group_id; ?>">
                    ...
                    <td>
                        ...
                        <?php echo \Extension\Html::anchor('admin/account-level/edit/' . $row->level_group_id, '<span class="glyphicon glyphicon-pencil"></span> ' . __('admin_edit'), array('class' => 'btn btn-default btn-xs' . (in_array($row->level_group_id, $disallowed_edit_delete) ? ' disabled' : null))); ?> 
                        <?php } ?> 
                    </td>
                </tr>
                <?php } // endforeach; ?> 
                <?php } else { ?> 
                <tr>
                    <td colspan="6"><?php echo __('fslang_no_data'); ?></td>
                </tr>
                <?php } // endif; ?> 
            </tbody>
        </table>
    ...
    <div class="row cmds">
        <div class="col-sm-6">
            <select name="act" class="form-control select-inline chosen-select">
                ...
                <?php if (\Model_AccountLevelPermission::checkAdminPermission('accountlv_perm', 'accountlv_delete_perm')) { ?><option value="del"><?php echo __('admin_delete'); ?></option><?php } ?> 
            </select>
