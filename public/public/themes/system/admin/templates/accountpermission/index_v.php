<h1><?php echo __('acperm_user_permission'); ?></h1>

<div class="row cmds">
    <div class="col-sm-6">
        <?php if ($account_check_result === true) { ?> 
        <button type="button" class="btn btn-danger" onclick="return ajaxResetPermission();"><?php echo __('acperm_reset_permission'); ?></button>
        <?php } // endif ?> 
        <div class="btn-group">
            <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"><?php echo __('acperm_module_permissison'); ?> <span class="caret"></span></button>
...
<script type="text/javascript">
    ...
    {
        var confirm_del = window.confirm('<?php echo __('acperm_are_you_sure_to_reset_for_this_user'); ?>');
        ...
        if (confirm_del == true) {
            $.ajax({
                ...
                success: function(data) {
                    if (data.result == true) {
                        alert('<?php echo __('acperm_reset_completed'); ?>');
                        location.reload();
                    } else {
                        alert('<?php echo __('acperm_failed_to_reset_permission'); ?>');
                        return false;
                    }
                }
            });
