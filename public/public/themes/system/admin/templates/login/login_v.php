...
<div class="alert alert-block alert-warning browser-alert browser-alert-<?php echo $browser_check; ?>">
	<?php 
	if ($browser_check == 'no') {
		echo \Lang::get('admin_get_modern_browser');
	} else {
		echo \Lang::get('admin_using_unknow_browser');
	}
	?> 
</div>
			...
			<div class="admin-login-page-layout">
				<div class="login-block">
					...
					<?php echo \Form::open(array('action' => \Uri::main() . '?rdr=' . $go_to, 'class' => 'form-horizontal', 'role' => 'form', 'onsubmit' => 'return ajaxAdminLogin($(this));')); ?> 
						<noscript><div class="alert alert-danger"><?php echo \Lang::get('admin_please_enable_javascript'); ?></div></noscript>
						...
						<div class="form-group">
							<label for="account_password" class="sr-only"><?php echo __('account_password'); ?>: <span class="txt_require">*</span></label>
							<div class="col-sm-12">
								<?php echo \Form::password('account_password', (isset($account_password) ? $account_password : ''), array('placeholder' => __('account_password'), 'id' => 'account_password', 'maxlength' => '70', 'class' => 'form-control')); ?> 
							</div>
						</div>
						<div class="form-group captcha-form-group<?php if (isset($show_captcha) && $show_captcha == true) { ?> show<?php }; ?>">
							<label for="account_captcha" class="sr-only"><?php echo __('account_captcha'); ?>: <span class="txt_require">*</span></label>
							<div class="col-sm-12">
						...
						<div class="form-group">
							<div class="col-sm-12">
								<button type="submit" class="btn btn-primary btn-block admin-page-login-btn"><span class="ajax-admin-login-processing"></span> <?php echo __('account_login'); ?></button>
				...
				<div class="requirement-check-block">
					<span><?php echo \Lang::get('admin_web_browser'); ?>: <small class="glyphicon glyphicon-<?php if (isset($browser_check)) {echo str_replace(array('yes', 'unknow', 'no'), array('ok', 'exclamation-sign', 'remove'), $browser_check);} ?>"></small></span>
					<span><?php echo \Lang::get('admin_javascript'); ?>: <small class="glyphicon glyphicon-remove" id="login-page-js-check"></small></span>
				</div><!--.requirement-check-block-->
				
				<div class="forgot-user-pw-block">
					<?php echo \Html::anchor('account/forgotpw', \Lang::get('account_forgot_username_or_password'), array('class' => 'btn btn-default btn-block')); ?> 
				</div>
